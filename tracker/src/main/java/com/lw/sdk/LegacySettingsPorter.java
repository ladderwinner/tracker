...
public class LegacySettingsPorter {
    static final String LEGACY_PREF_OPT_OUT = "LadderWinner.optout";
    ...
    public LegacySettingsPorter(@NonNull LadderWinner LadderWinner) {
        mLegacyPrefs = LadderWinner.getPreferences();
    }

    public void port(LWTracer LWTracer) {
        SharedPreferences newSettings = LWTracer.getPreferences();
        if (mLegacyPrefs.getBoolean(LEGACY_PREF_OPT_OUT, false)) {
            newSettings.edit().putBoolean(
                    LWTracer.PREF_KEY_TRACKER_OPTOUT,
                    true
            ).apply();
        ...
        if (mLegacyPrefs.contains(LEGACY_PREF_USER_ID)) {
            newSettings.edit().putString(
                    LWTracer.PREF_KEY_TRACKER_USERID,
        ...
        if (mLegacyPrefs.contains(LEGACY_PREF_FIRST_VISIT)) {
            newSettings.edit().putLong(
                    LWTracer.PREF_KEY_TRACKER_FIRSTVISIT,
        ...
        if (mLegacyPrefs.contains(LEGACY_PREF_VISITCOUNT)) {
            newSettings.edit().putLong(
                    LWTracer.PREF_KEY_TRACKER_VISITCOUNT,
        ...
        if (mLegacyPrefs.contains(LEGACY_PREF_PREV_VISIT)) {
            newSettings.edit().putLong(
                    LWTracer.PREF_KEY_TRACKER_PREVIOUSVISIT,
