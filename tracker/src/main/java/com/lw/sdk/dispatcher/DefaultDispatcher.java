...
public class DefaultDispatcher implements Dispatcher {
    private static final String TAG = LadderWinner.tag(DefaultDispatcher.class);
    ...
    private boolean launch() {
        synchronized (mThreadControl) {
            if (!mRunning) {
                ...
                thread.setName("LadderWinner-default-dispatcher");
    ...
    @Override
    public void submit(TraceMe traceMe) {
        mEventCache.add(new Event(traceMe.toMap()));
        if (mDispatchInterval != -1) launch();
    }
