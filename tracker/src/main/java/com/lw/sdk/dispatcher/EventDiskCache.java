...
public class EventDiskCache {
    private static final String TAG = LadderWinner.tag(EventDiskCache.class);
    ...
    public EventDiskCache(LWTracer LWTracer) {
        mMaxAge = LWTracer.getOfflineCacheAge();
        mMaxSize = LWTracer.getOfflineCacheSize();
        File baseDir = new File(LWTracer.getLadderWinner().getContext().getCacheDir(), CACHE_DIR_NAME);
        try {
            mCacheDir = new File(baseDir, new URL(LWTracer.getAPIUrl()).getHost());
