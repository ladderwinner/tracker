...
public class CustomVariablesTest extends BaseTest {
    ...
    public void testInject() throws Exception {
        ...
        cv.injectVisitVariables(traceMe);
        assertEquals(cv.toString(), traceMe.get(QueryParams.VISIT_SCOPE_CUSTOM_VARIABLES));
    }
    ...
    public void testToTrackMe() throws Exception {
        CustomVariables cv = new CustomVariables();
        cv.put(1, "name", "value");
        TraceMe traceMe = cv.toVisitVariables();
        assertEquals(cv.toString(), traceMe.get(QueryParams.VISIT_SCOPE_CUSTOM_VARIABLES));
    }
    ...
    public void testVisitCustomVariables() throws Exception {
        ...
        final TraceMe traceMe = LWTraceUtil.trace(visitVars.toVisitVariables())
                .screen("/path")
                .variable(1, "screen", "valueY")
                .build();

        assertEquals(visitVars.toString(), traceMe.get(QueryParams.VISIT_SCOPE_CUSTOM_VARIABLES));
        assertEquals(_screen.toString(), traceMe.get(QueryParams.SCREEN_SCOPE_CUSTOM_VARIABLES));
        assertEquals("/path", traceMe.get(QueryParams.URL_PATH));
    }
