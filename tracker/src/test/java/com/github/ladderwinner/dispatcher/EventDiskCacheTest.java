...
public class EventDiskCacheTest extends BaseTest {
    ...
    public void setup() throws Exception {
        ...
        when(mLWTracer.getAPIUrl()).thenReturn("http://testhost/LadderWinner.php");

        when(mLWTracer.getOfflineCacheAge()).thenReturn(0L);

        mCacheFolder = new File(mBaseCacheDir, "piwik_cache");
        mHostFolder = new File(mCacheFolder, "testhost");

        mDiskCache = new EventDiskCache(mLWTracer);
    }
    ...
    public void testMaxAge_positive_allStale() {
        when(mLWTracer.getOfflineCacheAge()).thenReturn(10 * 1000L);
        mDiskCache = new EventDiskCache(mLWTracer);
        Event event1 = new Event(1, "test");
        Event event2 = new Event(2, "test");
        mDiskCache.cache(Arrays.asList(event1, event2));
    ...
    public void testMaxAge_negative_cachingDisabled() {
        when(mLWTracer.getOfflineCacheAge()).thenReturn(-1L);
        mDiskCache = new EventDiskCache(mLWTracer);
    ...
    public void testMaxSize_limited() {
        when(mLWTracer.getOfflineCacheSize()).thenReturn(1024L);
        mDiskCache = new EventDiskCache(mLWTracer);
        for (int j = 0; j < 4; j++) {
    ...
    public void testMaxSize_disabled() {
        when(mLWTracer.getOfflineCacheSize()).thenReturn(0L);
        mDiskCache = new EventDiskCache(mLWTracer);
        for (int j = 0; j < 10; j++) {
            List<Event> events = new ArrayList<>();
    ...
    public void testOfflineMode_issue_271() {
        when(mLWTracer.getOfflineCacheSize()).thenReturn(4096L);
        mDiskCache = new EventDiskCache(mLWTracer);
